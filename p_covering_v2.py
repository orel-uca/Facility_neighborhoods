def optimiza(datosrec, ficheroArcs, n, p, radio, pc, tiempo, cortes, heur, pre):
    ...
    dis = mo.addVars([(k, l) for k, l in arcs], lb=0.0, name="dis")  # OPC1
    ...
    mo.setObjective(quicksum(W[0][i]*(y[i]+quicksum(x[i,j] for j in R if j != i)) for i in R), GRB.MAXIMIZE)
    ...
    mo.addConstrs(((1 >= (y[i] + quicksum(h[k, l] for k, l in arcs if k == i))) for i in R), name='R44')
    ...
    mo.addConstrs(((dd[i] <= radio*quicksum(h[k,j] for k, j in arcs if k == i)) for i in R), name='R72')

    # --- Constraints for SOC of euclidean distances between regions of R42 ---
    mo.addConstrs(((rz[k, l] <= dis[k, l] + bigM*(1-h[k, l])) for k, l in arcs), name='R42_0') # OPC 1 # The paper says <=, but in all studies we used ==
