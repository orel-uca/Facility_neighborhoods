def optimiza(datosrec, ficheroArcs, n, p, tiempo, cortes, heur, pre):
    ...
    # wi continuous variables represent points of the disk.
    ...
    # dd_i distances from client i to its service (whatever it is)
    dd = mo.addVars([(i) for i in R], lb=0.0, name="dd")

    # dis_kl distances between zk and zl
    dis = mo.addVars([(k, l) for k, l in arcs], lb=0.0, name="dis") # OPC 1
    ...
    # ----------------- zi belongs to Ri ------------------------
    # R is the number of regions. RP[i] is the number of polygons in region i
    ...
    mo.addConstrs((u[i, j, d] <= data[i, (2*n)*j+n+d]*t[i, j] for i in R if RP[i] != 0 for j in range(RP[i]) for d in N), name='R2_ur')  # In 3D it would be R2_tur (diagonal opposite to ll)
    ...
    mo.addConstrs(((dd[i] >= dd[j] + dis[i, j] - (r-p)*bigM*(1 - h[i, j])) for i, j in arcs), name='R42')
    mo.addConstrs((((quicksum(x[i, j] for j in R if j != i)) == (quicksum(h[k, l] for k, l in arcs if k == i))) for i in R), name='R43')
    mo.addConstrs(((1 == (y[i] + quicksum(h[i, l] for k, l in arcs if k == i))) for i in R), name='R44')
    mo.addConstrs(((h[k, i] <= ((quicksum(h[j, l] for j, l in arcs if l != k and j == i)) + y[i])) for k, i in arcs), name='R45')
    mo.addConstrs((x[i, k] >= h[i, j] + x[j, k] - 1 for i, j in arcs for k in R if i != k and j != k), name='R46')
    mo.addConstrs((x[i, j] >= h[i, j] + y[j] - 1 for i, j in arcs), name='R47')

    # --- Constraints for SOC of euclidean distances between regions in R42 ---
    mo.addConstrs(((rz[k, l] == dis[k, l] + bigM*(1-h[k, l])) for k, l in arcs), name='R42_0')  # Studies were done with ==, but in the paper it indicates <=
    mo.addConstrs((zkl[k, l, d] >= z[k, d] - z[l, d] for k, l in arcs for i in R for d in N), name='R42_1')
    mo.addConstrs((zkl[k, l, d] >= -z[k, d] + z[l, d] for k, l in arcs for i in R for d in N), name='R42_2')

    for k, l in arcs:
            mo.addQConstr((quicksum(zkl[k, l, d]*zkl[k, l, d] for d in N) <= rz[k, l]*rz[k, l]), name='R42_3')
    ...
    if mo.status == 4:
        ...
        print()
        print('{:17} {:5} {:5} {:3d} {:2d}  {:5.2f}  {:3d}  {:7.1f}    {:5.1f}  {:5.1f}  Infeasible-Unbounded'.format(modelo, Ej_dat, Ej_arc, r, p, dens, tiempo, cortes, heur, pre), file=outfile)
        return
