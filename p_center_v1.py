def optimiza(datosrec, ficheroArcs, n, p, tiempo, cortes, heur, pre):
    ...
    # f_ikl=1 if arc (k,l) is in the path from i to its p-center
    ...
    mo.addConstrs(((Zmin >= (quicksum(dis[i, k, l] for k, l in arcs))) for i in R), name='R51')
    mo.addConstrs(((1 == (y[i]+(quicksum(x[i, j] for j in R if j != i)))) for i in R), name='R52') # Studies were done with ==, but the paper indicates >=
    mo.addConstrs((-x[i, hk] == ((quicksum(f[i, k, l] for k, l in arcs if k != i and k == hk) - quicksum(f[i, l, k] for l, k in arcs if k != i and k == hk))) for i in R for hk in R if i != hk), name='R53')
    mo.addConstrs(((1-y[i] == (quicksum(f[i, k, l] for k, l in arcs if k == i))) for i in R), name='R54')
    mo.addConstrs(((1-y[kh] >= (quicksum(f[i, k, l] for k, l in arcs if k == kh))) for i in R for kh in R if i != kh), name='R55')

    # --- Constraints for SOC of euclidean distances between regions ---
    mo.addConstrs(((rz[i, k, l] == dis[i, k, l] + bigM*(1-f[i, k, l])) for i in R for k, l in arcs), name='R51_0')  # Studies were done with ==, but the paper indicates <=
    mo.addConstrs((zkl[i, k, l, d] >= z[k, d] - z[l, d] for i in R for k, l in arcs for d in N), name='R51_1')
    mo.addConstrs((zkl[i, k, l, d] >= -z[k, d] + z[l, d] for i in R for k, l in arcs for d in N), name='R51_2')

    for i in R:
        for k, l in arcs:
            mo.addQConstr((quicksum(zkl[i, k, l, d]*zkl[i, k, l, d] for d in N) <= rz[i, k, l]*rz[i, k, l]), name='R51_3')
